...
export default function TaskList() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ client_id: '', descrizione: '', stato: 'aperto' })
      setShowForm(false)
      fetchData()
    } catch (err) {
      setError(err.message)
  ...
  const getStatoColor = (stato) => {
    switch (stato) {
      ...
      case 'aperto': return 'bg-yellow-100 text-yellow-800'
          ...
          <tbody>
            ...
            {tasks.map(task => (
              <tr key={task.id} className="border-b hover:bg-gray-50">
                <td className="px-6 py-4 text-sm text-gray-900">{task.descrizione}</td>
